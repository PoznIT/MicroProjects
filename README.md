...
│   │   ├── common.py   shared {"error": ...} envelope + subprocess runner
│   │   └── routers/    ytaudio.py · timepunch.py
│   └── Dockerfile      python3-slim + yt-dlp + ffmpeg + deno
├── web/                Vite + React SPA
│   ├── src/
│   │   ├── App.jsx     router + theme
│   │   ├── pages/      Home · YTAudio · TimePunch
...
The unified API lives under `/api/<tool>/…` (e.g. `/api/ytaudio/download`).
TimePunch is client-side; its router only exposes a health check.

## Adding a tool

1. Add a router under `backend/app/routers/` (prefix `/api/<tool>`) and include it in `main.py`. Reuse `common.py` for error envelopes and subprocess spawning.
